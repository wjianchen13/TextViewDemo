 ...
 * 测试
 */
public class FinalTestAdapter6 extends BaseMultiItemQuickAdapter<FinalTestBean6, BaseViewHolder> {
    ...
    public FinalTestAdapter6(Context context, List<FinalTestBean6> list) {
        super(list);
        this.mContext = context;
        addItemType(TYPE_TEXT, R.layout.item_final_test6_text);
        addItemType(TYPE_GRADIENT, R.layout.item_final_test6_gradient);
    }

    @Override
    protected void convert(BaseViewHolder helper, FinalTestBean6 item) {
        if (item == null) {
            return;
        }
...
//            tvTest.setContent(item.getsContent());
    ...
    public List<FinalTestBean6> getData() {
        return mData;
    }
