...
import com.example.textviewdemo.vertical.MyTextView;

/**
 * 当TextView的实际高度大于父控件的高度时，垂直显示TextView的中间部分
 */
public class GradientVerticalTextView extends FrameLayout {

    private Context mContext;
    private GradientTextView1 tvContent;
    private String mText;
    private int mTextSize;
    private ColorStateList mTextColor;
    private int mTextStyle;

    public GradientVerticalTextView(@NonNull Context context) {
        super(context);
        initAttrs(context, null);
        init(context);
    }

    public GradientVerticalTextView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initAttrs(context, attrs);
        init(context);
    }

    public GradientVerticalTextView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        ...
        LayoutInflater.from(mContext).inflate(R.layout.view_gradient_vertical, this);
