 ...
 * 字体渐变动画
 */
class GradientTextView extends AppCompatTextView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (translateAnimate) {
            if (direction == leftToRight) {
                translate += getMeasuredWidth() / translateSpeed;
        ...
        switch (direction) {
            case leftToRight:
                if (translateAnimate) {
                    ...
                    mLinearGradient = new LinearGradient(0f, 0f, getMeasuredWidth(), 0f, startColor, endColor, Shader.TileMode.CLAMP);
                }
                break;
            case topToBottom:
                if (translateAnimate) {
